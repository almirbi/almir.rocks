...
class Game {
  ...
  getBricks() {
    return Array.prototype.slice.call(
      this.gameWindow.querySelectorAll(".icons > .icon")
    );
  }
  ...
  handleStart(event) {
    ...
    if (event.button == 0 || event.button === undefined) {
      ...
      this.removeMessageTimeout = setTimeout(() => {
        ...
        explanation.style.setProperty(
          "height",
          explanation.offsetHeight + "px"
        );
  ...
  resetGame() {
    if (this.state == State.LOST) {
      ...
      this.ball.style.left =
        ...
        "px";
      this.ball.style.top =
        this.getElementOffsetTop(this.pad.parentElement) -
  ...
  bounce() {
    ...
    if (
      this.pad.parentElement.offsetTop + 10 <
      this.ball.offsetTop + this.ball.offsetHeight
    ) {
      throw "Lost";
    }
    ...
    if (
      this.getElementOffsetLeft(this.ball) >=
        x - parseInt(this.ball.offsetWidth) &&
      this.direction.x === 1
    ) {
    ...
    if (
      this.getElementOffsetTop(this.ball) + 21 >=
        y - parseInt(this.ball.offsetHeight) &&
      this.direction.y === -1
    ) {
      if (
        this.getElementOffsetLeft(this.ball) >
          this.getElementOffsetLeft(this.pad) - this.ball.offsetWidth - 5 &&
        this.getElementOffsetLeft(this.ball) <
          this.getElementOffsetLeft(this.pad) + this.pad.offsetWidth
      ) {
        this.direction.y = 1;
        console.log("pad!!!");
        return "pad";
  ...
  victory() {
    ...
    this.ball.style.setProperty(
      "background-image",
      `url('${require("./assets/images/victory.jpg").default}')`
    );
  ...
  moveBall() {
    try {
      ...
      for (let i = 0; i < this.settings.ballSpeed; i++) {
        ...
        let left =
            this.getElementOffsetLeft(this.ball) + this.direction.x + "px",
        ...
        let ballTop = this.getElementOffsetTop(this.ball),
          ...
          topThreshold =
            this.getElementOffsetTop(lastIcon) + lastIcon.offsetHeight;
